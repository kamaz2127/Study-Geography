...
#функция начала игры(отрисовывает список спрайтов и здесь будут функции самой игры)
...
while game:
    ...
    for e in pygame.event.get():
            if e.type == pygame.QUIT:
                game = False
            if e.type == pygame.MOUSEBUTTONDOWN:  #ниже метод в котором у меня проблема. 
                x,y = pygame.mouse.get_pos()      #Обозначаю переменную как координаты мыши.
                for count in l_country:             #перебор списка
                    if count.rect.collidepoint(x,y): #если спрайт коснулся координат мыши то (раньше работало исправно, теперь реагирует и на фон, вообще не понимаю почему)
                        print(count.name_counrty())     #печатает название страны по которой щелкнули
                        break                              #проблема в том что печатает неправильное название, подозреваю что перебор просто осуществляется пока не нажмут мышкой, не понимаю почему не возвращает правильное название
